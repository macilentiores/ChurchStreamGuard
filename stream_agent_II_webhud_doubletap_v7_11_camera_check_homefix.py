"""
stream_agent_II_webhud_doubletap.py

Stream Agent II — PC HUD + Web HUD (tablet/phone)

Version: v7.11 — Quiet HUD + Self-Healing + Persistent Logs
...
- FIX: Camera-issue detection no longer references undefined variables; it now uses stream_stable_since/start_time
       and inspects the camera source check result dict.
- FIX: Auto-recover start grace window now applies even after a recovery cycle is armed (prevents rapid double-start).
- CLEANUP: Removed duplicate assignments in _reset_recovery_state.
- NEW: Web HUD "Health" strip above the log (sticky status + last critical error; does not scroll away)
- NEW: Optional auto-recovery: if streaming stops unexpectedly, attempt restart with backoff (rate-limited)
- NEW: Log-to-file: write a full "run" log and optional per-stream session logs beside this script (configurable)
- CHANGE: PC HUD auto-restore / bring-to-front behaviours are now config-gated and default OFF (no popups)
- FIX: MIDI note handling now triggers on NOTE_ON velocity>0 only (ignores NOTE_OFF / NOTE_ON vel=0 to prevent double-triggers)
- FIX: Auto-recovery adds a start grace window to avoid immediate double-start retries while OBS transitions to streaming=true
- Existing: Per-preset delays for MIDI/automation are feature-gated (HUD preset buttons remain immediate)
...
class Config:
    ...
    Acronyms (first use):
      - OBS = Open Broadcaster Software (the streaming program)
      - NDI = Network Device Interface (network video source)
      - MIDI = Musical Instrument Digital Interface (control messages from Proclaim/loopMIDI)
      - VISCA = camera control protocol (used here over IP/UDP)
      - LAN = Local Area Network (your church AV network)

    Notes:
      - PC HUD uses Tkinter: iconify()=minimize, deiconify()=restore.
      - For church operation: keep popups OFF and rely on Web HUD status + logs.
    """

    # ----------------------------
    # MODE / ENVIRONMENT
    # ----------------------------
    # True = safe at-home testing (camera commands are simulated; no VISCA packets sent).
    # False = church/live mode (camera power/presets are actually sent).
    HOME_TEST_MODE: bool = True


    # ----------------------------
    # PC HUD WINDOW BEHAVIOUR (LOCAL SCREEN)
    # ----------------------------
    # Auto-minimize keeps the local PC screen clean once streaming is stable.
    # The Web HUD is the primary monitoring surface during service.
    AUTO_MINIMIZE_ENABLED: bool = True
    AUTO_MINIMIZE_AFTER_SECONDS: int = 5



    # If True, the PC HUD will minimize shortly after launch even if you are not streaming yet.
    # Uses AUTO_MINIMIZE_AFTER_SECONDS as the launch delay.
    MINIMIZE_ON_STARTUP: bool = True
    # Pop-up behaviour controls (defaults are OFF).
    # - AUTO_RESTORE_ON_ISSUE: if minimized, restore on detected issues (unexpected stop, OBS error, camera issue).
    # - AUTO_BRING_TO_FRONT_ON_STREAM_START: restore when stream becomes live.
    # For a single busy operator, leave these False so the HUD never steals focus.
    AUTO_RESTORE_ON_ISSUE: bool = False  # if True, restores minimized window on detected issues
    AUTO_BRING_TO_FRONT_ON_STREAM_START: bool = False  # if True, restores window when stream becomes live


    # ----------------------------
    # AUTO-RECOVERY (SELF-HEALING)
    # ----------------------------
    # If the stream stops unexpectedly while we believe we *should* be live,
    # try to restart streaming automatically, with backoff + a cooldown to avoid loops.
    # NOTE: This cannot resurrect OBS if the OBS process has crashed; it can only retry via OBS WebSocket.
    ...
    AUTO_RECOVER_COOLDOWN_SECONDS: int = 300  # pause before another restart cycle
    # Grace window after any StartStream request to avoid double-starting while OBS transitions to streaming=true.
    AUTO_RECOVER_START_GRACE_SECONDS: int = 15  # wait before retrying StartStream after a start request


    # ----------------------------
    # LOGGING (PERSISTENT FILES)
    # ----------------------------
    # Logs are written beside this script by default (LOG_DIR='').
    # - Run log: one file per app launch
    # - Session log: optional file created when a stream session starts (recommended for post-service review)
    LOG_TO_FILE_ENABLED: bool = True
    LOG_RUN_FILE_PREFIX: str = "stream_agent"
    LOG_SEPARATE_SESSION_FILES: bool = True  # create a new log file when a stream session starts
    LOG_DIR: str = ""  # blank = store logs beside this .py file


    # ----------------------------
    # OBS CONNECTION (OBS WEBSOCKET)
    # ----------------------------
    # OBS WebSocket (built-in on modern OBS) must match host/port/password here.
    OBS_HOST: str = "127.0.0.1"
    OBS_PORT: int = 4455
    OBS_PASSWORD: str = ""


    # Camera source check inside OBS (optional).
    # Used to flag 'camera missing/hidden' while live (feeds the Web HUD health state).
    # Set OBS_CAMERA_INPUT_NAME if you know the exact input name, otherwise the NDI sender scan is used.
    ...
    # Enable/disable OBS camera-source monitoring.
    # - Leave enabled at church so the Web HUD can warn if the camera source is missing/hidden.
    # - Default: disabled in HOME_TEST_MODE to avoid confusing warnings when no camera exists.
    CAMERA_SOURCE_CHECK_ENABLED: bool = True
    CAMERA_SOURCE_CHECK_IN_HOME_TEST: bool = False


    # Stream safety controls.
    # STOP_DELAY_SECONDS adds a countdown before issuing Stop (helps prevent accidental stops).
    # START_DEBOUNCE_SECONDS ignores repeated Start presses within the window.
    STOP_DELAY_SECONDS: int = 30
    START_DEBOUNCE_SECONDS: float = 5.0


    # ----------------------------
    # WEB HUD (REMOTE MONITORING)
    # ----------------------------
    # This is the primary 'operator view' on a tablet/phone on the same LAN.
    # WEB_HUD_TOKEN can be set to require ?token=XYZ in the URL (basic shared-secret protection).
    ...
    # ----------------------------
    # CAMERA_IP / CAMERA_VISCA_PORT must match the PTZ camera network settings.
    # PRESET_NUMBER_BASE is an offset applied to preset recall:
    #   - 0 means preset 1 -> camera preset 1 (most common)
    #   - 1 means preset 1 -> camera preset 2 (if camera numbering is shifted)
    ...
    # ----------------------------
    # PRESET DELAYS (MIDI/AUTOMATION ONLY)
    # ----------------------------
    # ENABLE_PRESET_DELAYS applies only to MIDI/automation preset recalls.
    # HUD preset buttons remain immediate (operator judgement; no programmed delays).
    ...
    PRESET_DELAYS_SECONDS: Dict[int, int] = field(default_factory=lambda: {
        ...
        4: 12,   # Altar
        5: 20,   # Choir (suggested starting point: 10-20)
        6: 0,   # Screen
        7: 0,   # Band
        8: 0,   # Piano
        9: 30,   # Communion
    ...
    # ----------------------------
    # MIDI INPUT (FROM PROCLAIM / loopMIDI)
    # ----------------------------
    # We scan available MIDI ports and connect to the first name containing this substring.
    # MIDI_CHANNEL_1_BASED uses human-friendly numbering (1..16).
    MIDI_INPUT_PORT_SUBSTRING: str = "proclaim"
    MIDI_CHANNEL_1_BASED: int = 1


    # MIDI note map (these are the note numbers Proclaim/your MIDI source sends).
    # NOTE_PRESET_FIRST..LAST define a contiguous range that maps to presets 1..10.
    ...
    # ----------------------------
    # If enabled, the agent can auto-start streaming at a set time on a given weekday.
    # TIMER_WEEKDAY uses Python's weekday(): Monday=0 ... Sunday=6.
    ...
    # Time zone fallback (only used if Python's zoneinfo is unavailable).
    # - 'local' uses the PC local time zone.
    # - 'fixed_offset' uses TZ_FALLBACK_UTC_OFFSET_HOURS.
    TZ_FALLBACK_MODE: str = "local"
    TZ_FALLBACK_UTC_OFFSET_HOURS: int = -6


    # Human-friendly labels for buttons/log lines (preset numbers 1..10).
...
"""
    def _web_js(self) -> str:
        ...
        return r"""(function(){
  'use strict';
  // Stream Agent HUD JS v2.0 (schema + newline fix)
  try { console.log('Stream Agent HUD JS v2.0 loaded'); } catch (e) {}
...
var wsReady = false;
  ...
  connect();
    ...
    def _on_close(self):
        ...
        # Close any open log files cleanly
        try:
