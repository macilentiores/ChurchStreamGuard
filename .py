...
class Config:
    ...
    TIMEZONE: str = "America/Regina"  # used for timer countdown/start

    # If TIMEZONE can't be loaded (common on Windows if the optional "tzdata" package is missing),
    # fall back so the app still runs even without internet.
    # - "local": use the PC's local clock/time zone
    # - "fixed_offset": use a fixed UTC offset (e.g., -6 for Regina CST year-round)
...
class App:
    def __init__(self, cfg: Config):
        ...
        # Pending start handling (wait for camera/OBS, then auto-start)
        self._pending_stream_start: bool = False
        self._pending_stream_start_source: Optional[str] = None
        self._last_pending_obs_post_at: float = 0.0

        # Timezone for timer (Regina)
        # Time zone handling:

        # Prefer IANA tz (e.g., "America/Regina"). If that fails (often missing tzdata on Windows),

        # fall back per config so the app keeps running.
        ...
        if ZoneInfo:
            ...
            try:
                ...
                self._tz_note = self.cfg.TIMEZONE

            except Exception as e:

                # fallback

                if str(getattr(self.cfg, "TZ_FALLBACK_MODE", "local")).lower().startswith("fixed"):

                    self._tz = dt.timezone(dt.timedelta(hours=int(getattr(self.cfg, "TZ_FALLBACK_UTC_OFFSET_HOURS", -6))))

                    self._tz_note = f"fixed UTC{int(getattr(self.cfg, 'TZ_FALLBACK_UTC_OFFSET_HOURS', -6)):+d} (ZoneInfo failed: {e})"

                else:

                    self._tz = None  # local system clock

                    self._tz_note = f"local clock (ZoneInfo failed: {e})"

        else:

            self._tz_note = "local clock (ZoneInfo not available)"
        # Tkinter HUD
        self.root = tk.Tk()
    ...
    def _build_ui(self):
        ...
        ttk.Label(frm, textvariable=self.var_status).grid(row=1, column=0, sticky="w", pady=(4, 2))

        ttk.Label(frm, textvariable=self.var_countdown).grid(row=2, column=0, sticky="w", pady=(0, 6))
        ...
        ttk.Separator(frm).grid(row=6, column=0, sticky="ew", pady=8)
        ...
        presets.grid(row=8, column=0, sticky="ew", pady=(8, 0))
        ...
        ttk.Label(frm, text="Last:").grid(row=10, column=0, sticky="w")
        ttk.Label(frm, textvariable=self.var_last, wraplength=360).grid(row=11, column=0, sticky="w")
    ...
    def _ui_fire(self, action: str):
        ...
        if action == "start_stream":
            ...
            self._request_start_stream(source="HUD")
    ...
    def _send_obs_start(self, source: str):
        try:
    ...
    def _request_start_stream(self, source: str):
        """Request a stream start.

        Called from TIMER, MIDI, or the HUD button. It will:
        - debounce repeated triggers (e.g., Proclaim loop spam),
        - ensure camera wake begins first,
        - set a pending flag so we can automatically start once camera/OBS are ready.
        """
        ...
        # Remember why we want to start, so the loop can finish the job later
        self._pending_stream_start = True
        self._pending_stream_start_source = source

        # Begin camera wake if needed
        if self.camera_state == "SLEEP":
            self._wake_camera(source=f"{source}/start")

        # Try immediately (may succeed if everything is already ready)
        self._maybe_start_stream()

    def _maybe_start_stream(self):
        """If a start is pending and prerequisites are ready, actually start OBS."""
        if not getattr(self, "_pending_stream_start", False):
            return

        # Wait for camera readiness (unless home test mode)
        if not self._camera_is_ready():
            return

        # Wait for OBS connection (auto reconnect runs in loop)
        if not self.obs.is_connected():
            # Don't spam the log every 0.1s
            if (time.time() - self._last_pending_obs_post_at) > 5.0:
                self._last_pending_obs_post_at = time.time()
                src = getattr(self, "_pending_stream_start_source", "PENDING")
                self.post(f"{src}: waiting for OBS connection…")
            return

        # Fire!
        src = getattr(self, "_pending_stream_start_source", "PENDING")
        self._pending_stream_start = False
        self._pending_stream_start_source = None
        self._send_obs_start(source=src)
    ...
    def _now_local(self) -> dt.datetime:
        if getattr(self, "_tz", None):
            return dt.datetime.now(self._tz)  # type: ignore[arg-type]
        return dt.datetime.now()

    def _parse_hhmm(self) -> Optional[Tuple[int, int]]:
        try:
            hh_s, mm_s = self.cfg.TIMER_START_HHMM.split(":")
            hh = int(hh_s)
            mm = int(mm_s)
    ...
    def _next_timer_target(self) -> Optional[dt.datetime]:
        """Return the next scheduled timer start datetime (always in the future), or None."""
        ...
        now = self._now_local()
        weekday = self.cfg.TIMER_WEEKDAY

        # Find the next date that matches the weekday
        days_ahead = (weekday - now.weekday()) % 7
        candidate = now + dt.timedelta(days=days_ahead)
        target = candidate.replace(hour=hh, minute=mm, second=0, microsecond=0)

        # If it's "today" but already passed (or we already fired today), push to next week
        if target <= now or (self.timer_fired_for_date == now.date() and now.weekday() == weekday):
            target = target + dt.timedelta(days=7)

        return target

    def _timer_countdown_text(self) -> str:
        if not self.cfg.USE_TIMER_START:
            return "Timer start: OFF"

        nxt = self._next_timer_target()
        if not nxt:
            return "Timer start: (invalid time)"
        ...
        # Display: countdown + scheduled time
        when = nxt.strftime("%a %H:%M")
        if days > 0:
            return f"Next auto-start: {when} (in {days}d {hh:02d}:{mm:02d}:{ss:02d})"
        return f"Next auto-start: {when} (in {hh:02d}:{mm:02d}:{ss:02d})"
    ...
    def _update_ui(self):
        ...
        # Timer countdown / confidence line
        self.var_countdown.set(self._timer_countdown_text())

        # If a stop is pending, show the countdown prominently
        if self.stop_pending_until:
            remaining = max(0, int(self.stop_pending_until - time.time()))
            self.var_status.set(f"Stop pending… {remaining}s")
        else:
            self.var_status.set(self.last_post)
    async def loop(self):
        self._pending_stream_start = False
        self._pending_stream_start_source = None
        self._last_pending_obs_post_at = 0.0
        ...
        while self.running:
            ...
            for msg in self.midi.poll():
                ...
                if self.midi.is_note_on(msg, self.cfg.NOTE_START_STREAM):
                    ...
                    self._request_start_stream(source="MIDI")
